...
export function CaseStudyPage({ projectId, onBack }: CaseStudyPageProps) {
  ...
  // Misma animación de "aparición al scrollear" que usa el Home
  useFadeIn([projectId])
  ...
  return (
    <main className="case-study page-enter" id="case-study-top">
      ...
      <ImageCarousel className="case-hero-image" images={caseStudy.heroImages} />

      {caseStudy.sections.map((section, index) => (
        <div key={`${caseStudy.id}-section-${index}`} className="fade-section">
          ...
          {section.images && section.images.length > 0 && (
            <ImageCarousel className="case-detail-image" images={section.images} />
          )}
        </div>
      ))}
    </main>
